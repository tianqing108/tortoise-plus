...
from tortoise.backends.sqlcipher.executor import SqlcipherExecutor
from tortoise.backends.sqlcipher.schema_generator import SqlcipherSchemaGenerator
from tortoise.connection import connections
from tortoise.contrib.sqlcipher.regex import (
...
class SqlcipherClient(BaseDBAsyncClient):
    executor_class = SqlcipherExecutor
    query_class = SQLLiteQuery
    schema_generator = SqlcipherSchemaGenerator
    ...
    def __init__(self, file_path: str, **kwargs: Any) -> None:
        ...
        self._connection: aiosqlcipher.Connection | None = None
        self._lock = asyncio.Lock()

    async def create_connection(self, with_db: bool) -> None:
        if not self._connection:  # pragma: no branch
            self._connection = aiosqlcipher.connect(self.filename, isolation_level=None)
            self._connection.start()
            await self._connection._connect()
            self._connection._conn.row_factory = sqlite3.Row
            if "key" in self.pragmas:
                cursor = await self._connection.execute(f"PRAGMA key={self.pragmas['key']}")
                await cursor.close()
                self.pragmas.pop("key")
            for pragma, val in self.pragmas.copy().items():
                if "cipher" not in pragma:
                    continue
                print(f"PRAGMA {pragma}={val}")
                cursor = await self._connection.execute(f"PRAGMA {pragma}={val}")
                await cursor.close()
                self.pragmas.pop(pragma)
...
class SqliteTransactionWrapper(SqlcipherClient, TransactionalDBClient):
    def __init__(self, connection: SqlcipherClient) -> None:
        self.capabilities = connection.capabilities
        self.connection_name = connection.connection_name
        self._connection: aiosqlcipher.Connection = cast(aiosqlcipher.Connection, connection._connection)
        self._lock = asyncio.Lock()
...
class SqliteClientWithRegexpSupport(SqlcipherClient):
    capabilities = Capabilities(
        "sqlcipher",
        daemon=False,
        requires_limit=True,
